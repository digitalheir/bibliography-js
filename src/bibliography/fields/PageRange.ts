...
export function isSinglePage(pr: PageRange): pr is string {
  return typeof pr === "string";
}

export function isPageRange(pr: PageRange): pr is [string, string] {
  return !isSinglePage(pr);
}

export function getPageStart(pr: PageRange): string {
  if (isSinglePage(pr)) {
    return pr;
  } else return pr[0];
}
