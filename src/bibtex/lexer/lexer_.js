    ...
    switch (this.mode) {
      case modes.OUTSIDE_ENTRY:
        if (this.str.charAt(this.pos) == '@') {
          ...
          return new Token(modes.OUTSIDE_ENTRY, chars.join("").trim());
        }
      case modes.ENTRY_TYPE:
        ...
        return new Token(modes.ENTRY_TYPE, this.entryType);
      case modes.CITATION_KEY:
        ...
        if (charAtCK == '}' || charAtCK == ',') {
          ...
          return new Token(charAtCK, charAtCK);
        } else return new Token(modes.CITATION_KEY, this.getStringUntilNonEscapedChar(',').trim());
      case modes.FIELD_KEY:
        ...
        if (charAtFK == '}' || charAtFK == '=') {
          ...
          return new Token(charAtFK, charAtFK);
        } else return new Token(modes.FIELD_KEY, this.getStringUntilNonEscapedChar('=').trim());
      case modes.FIELD_VAL:
        ...
        if (charAtFV == '}' || charAtFV == ',') {
          ...
          return new Token(charAtFV, charAtFV);
        } else return new Token(modes.FIELD_VAL, this.getStringUntilNonEscapedChar(/[,}]/).trim());
      ...
      case modes.COMMENT_ENTRY:
        ...
        if (this.str.charAt(this.pos) == '}') {
          ...
          return new Token(modes.COMMENT_ENTRY, this.getStringUntilNonEscapedChar('}'));
        }
      case modes.BIB_ENTRY:
        // Identifier
        return new Token(modes.COMMENT_ENTRY, "");
      default:
        throw new Error();
    }
  }
