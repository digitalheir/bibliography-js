...
function id(x) {return x[0]; }
  ...
  var tok_typedeclaration = {test: function(x) {return !!x.match(/@[\{]+/); }}
var grammar = {
    ParserRules: [
    ...
    {"name": "bibfile", "symbols": ["bibfile$ebnf$1", "bibfile$ebnf$2"], "postprocess":  function(data, location, reject) {
            objs = []
            for(var i in data[0]) {
                if(typeof data[0][i] != 'string' || data[0][i].trim().length > 0)
                    objs.push(data[0][i]);
            }
            if(data[1] && data[1].trim().length > 0) objs.push(data[1]);
            return objs;
        }
                                                              },
    ...
    {"name": "entry", "symbols": [{"literal":"@"}, "_", tok_typedeclaration, "_", "body"], "postprocess":  function (data, location, reject) {
            var obj = data[4];
            obj['@type'] = data[2].replace("@","").trim();
            return obj;
        }
                                                              },
    ...
    {"name": "fields", "symbols": ["fields$ebnf$1", "field"], "postprocess":  function (data, location, reject) {
               var o = {};
               function addTo(it){
                  if(!o[it[0]]) o[it[0]] = it[1];
                  else {
                    console.warn(it[0]+" was defined more than once");
                  }
               }
               for(var i in data[0]) addTo(data[0][i]);
               addTo(data[1]);
